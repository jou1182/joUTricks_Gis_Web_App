...
function handleFileUpload(files) {
    ...
    status.classList.remove('error-message'); // Clear previous error styling
...
function openStyleEditor(layerId) {
    ...
    if (!ld) return;    // NOTE: Using prompt() is generally discouraged for production.
    // For a better UX, a dedicated styling modal should be implemented.
    // For now, we'll keep the prompt() but add a warning.
    console.warn('Using prompt() for styling. Consider implementing a dedicated modal for production.');

    const newColor = prompt('اختر لون الطبقة (hex) مثل #FF0000:', ld.style.color);
    const newWeight = prompt('سماكة الخط (weight):', ld.style.weight);
    const newFillOpacity = prompt('إشفافية التعبئة (0-1):', ld.style.fillOpacity);
    const newRadius = prompt('حجم نقاط (radius):', ld.style.radius);
    const newShape = prompt('شكل النقاط: circle / marker', ld.style.shape || 'circle');

    // Validate and update style
    const color = newColor || ld.style.color;
    const weight = parseInt(newWeight, 10) || ld.style.weight;
    const fillOpacity = parseFloat(newFillOpacity) || ld.style.fillOpacity;
    const radius = parseInt(newRadius, 10) || ld.style.radius;
    const shape = newShape || ld.style.shape;

    // Check if any value was actually changed before re-adding the layer
    const newStyle = { color, weight, fillOpacity, radius, shape };
    const styleChanged = JSON.stringify(ld.style) !== JSON.stringify(newStyle);

    if (styleChanged) {
        // update style in object
        ld.style = newStyle;
        // remove and re-add layer
        map.removeLayer(ld.layer);
        // The addGeoJSONLayer function will create a new layer with a new ID.
        // We need to delete the old layer entry from the global 'layers' object after the new one is added.
        delete layers[layerId]; // Delete old entry before re-adding
        addGeoJSONLayer(ld.geojson, ld.name, ld.style);
    }
    updateLayersList();old layer entry from the global 'layers' object.
...
function displayFeatureInfo(properties, layerName) {
    ...
    let html = `<div class="feature-property"><strong class="property-key">الطبقة:</strong> <span class="property-value">${layerName}</span></div>`;
